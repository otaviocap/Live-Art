...
RUN apt update -y && apt install -y build-essential git software-properties-common npm curl \
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/live_art ./
